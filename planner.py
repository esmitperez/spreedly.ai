"""Spreedly Agent that interacts with OpenAPI APIs via a hierarchical planning approach."""
import json
import re
...
def _create_api_planner_tool(
    ...
    endpoint_descriptions = [
        # fix wrong structure exposed by ReducedOpenAPISpec 
        # f"{name} {description}" for name, description, _ in api_spec.endpoints
        f"{name} {description}" for name, description in api_spec['paths'].items()
    ]
...
def _create_api_controller_tool(
    ...
    def _create_and_run_api_controller_agent(plan_str: str) -> str:
        ...
        endpoint_names = [
            ...
            "{route}".format(method=method, route=route.split("?")[0])
            for method, route in matches
        ]
        # fix wrong structure exposed by ReducedOpenAPISpec 
        # endpoint_docs_by_name = {name: docs for name, _, docs in api_spec.endpoints}
        endpoint_docs_by_name = {name: docs for name, docs in api_spec['paths'].items()}
        docs_str = ""
